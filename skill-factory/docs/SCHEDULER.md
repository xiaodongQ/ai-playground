...
# 停止
curl -X POST localhost:8080/api/scheduler/stop
# 状态
curl localhost:8080/api/scheduler/status
# {"running": true}
# 重载（增删定时任务后自动调用，也可手动）
curl -X POST localhost:8080/api/scheduler/reload
```
...
```bash
curl -X POST localhost:8080/api/scheduled -H "Content-Type: application/json" -d '{
  "name": "heartbeat",
  "cron_expr": "@every 30s",
  "command_type": "shell",
...
```bash
# 不依赖 cron，立即跑一次
curl -X POST localhost:8080/api/scheduled/{id}/run-now
```
...
# 2. 看最近执行
curl 'localhost:8080/api/executions?limit=20' | jq '.[] | {command, exit_code, started_at, output}'

# 3. 看 last_status
curl localhost:8080/api/scheduled | jq '.[] | {name, last_status, last_run_at}'
```
