...
### List Experiences
```bash
curl -s http://localhost:8080/api/experiences[?module=redis-cluster]
```
Returns: `Experience[]`

### Get Experience by ID
```bash
curl -s http://localhost:8080/api/experiences/{id}
```
Returns: `Experience`
...
### Delete Experience
```bash
curl -s -X DELETE http://localhost:8080/api/experiences/{id}
```