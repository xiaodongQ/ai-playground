...
class Database:
    ...
    async def init(self):
        async with aiosqlite.connect(self.db_path) as db:
            # 迁移：添加 command 列到 executions（如果不存在）
            try:
                await db.execute("ALTER TABLE executions ADD COLUMN command TEXT")
            except Exception:
                pass  # 列已存在

            # 迁移：添加 exit_code 列到 executions（如果不存在）
            try:
                await db.execute("ALTER TABLE executions ADD COLUMN exit_code INTEGER")
            except Exception:
                pass  # 列已存在

            # 迁移：添加 session_id 列到 tasks（如果不存在）
            try:
                await db.execute("ALTER TABLE tasks ADD COLUMN session_id TEXT")
            except Exception:
                pass  # 列已存在

            # 迁移：添加 user_input_required 列到 tasks（如果不存在）
            try:
                await db.execute("ALTER TABLE tasks ADD COLUMN user_input_required INTEGER DEFAULT 0")
            except Exception:
                pass  # 列已存在

            # 迁移：添加 pending_input 列到 tasks（如果不存在）
            try:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            async with db.execute(
                ...
                return [Execution(
                    ...
                    command=r[3] if len(r) > 3 else None,
                    started_at=r[4], completed_at=r[5], output=r[6], error=r[7],
