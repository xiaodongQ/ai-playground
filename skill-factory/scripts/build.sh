...
# skill-factory 构建脚本
# 用法：
#   ./scripts/build.sh              # 默认：当前平台 → ./bin/skill-factory
#   ./scripts/build.sh -a           # 三平台：./bin/skill-factory-{darwin,linux,windows}-{amd64}.exe
...
Options:
  (无)        编译当前平台到 ${BIN_DIR}/skill-factory
...
build_current() {
  mkdir -p "$BIN_DIR"
  echo "==> 当前平台 → ${BIN_DIR}/skill-factory"
  go build "${LDFLAGS[@]}" -o "${BIN_DIR}/skill-factory" ./cmd/server
  ls -lh "${BIN_DIR}/skill-factory" | awk '{printf "    %s  %s\n", $5, $9}'
}

build_one() {
  local goos=$1 goarch=$2 ext=$3
  local bin_name="skill-factory-${VERSION}-${goos}-${goarch}${ext}"
