...
```bash
# 1. 备份
cp data/skill-factory.db data/skill-factory.db.v1.bak
...
# 3. 验证
sqlite3 data/skill-factory.db "PRAGMA user_version;"   # 应该是 2
sqlite3 data/skill-factory.db ".tables"                  # 应该看到 9 张表
```

## 完全重建（不保留数据）

```bash
rm data/skill-factory.db
./skill-factory  # 启动时自动建空 db
```
...
DB_PATH=/tmp/fresh.db /tmp/skill-factory
```
...
-- 4. 注入到 v2
sqlite3 data/skill-factory.db < v24-dump.sql
```
