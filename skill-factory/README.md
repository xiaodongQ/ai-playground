...
融合 `skill-factory v1`（Go + 经验库 + PTY + 漂亮 UI）和 `ai-task-system v2.4`（Python 调度 + AI CLI 执行器），加上 5 个新功能，作为日常 homepage / launcher / 任务调度统一入口。
...
# Linux
GOOS=linux GOARCH=amd64 go build -o skill-factory-linux ./cmd/server

# Windows
GOOS=windows GOARCH=amd64 go build -o skill-factory.exe ./cmd/server
```
...
`data/` 目录保留 6 份历史快照：
- `skill-factory.db.v1.bak` — v1 旧库
- `skill-factory.db.v2.fresh.db` — v2 全新 schema 空库
- `skill-factory.db.v2.phase2.db` / `phase3.db` / `phase3-ui.db` / `phase4-modal.db` — 各阶段验证库
