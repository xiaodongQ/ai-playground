...
func (s *APIServer) handlePty(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		conn.WriteMessage(websocket.TextMessage,
			[]byte("\r\n\x1b[31m[skill-factory] PTY 启动失败: "+err.Error()+"\x1b[0m\r\n"))
		return
	}
	defer ptmx.Close()
	defer cmd.Process.Kill()

	banner := fmt.Sprintf("\x1b[36m[skill-factory] PTY 启动 (shell=%s, cli=%s)\x1b[0m\r\n", shell, cliType)
...
// ctxDir 存在时读取 .skill-factory/context/*.md 作为 prompt-file。
